...
**Оговорка после ADR 0002.** Этот довод применяется только если mesh
нужен. ADR 0002 зафиксировал требования — связность и шифрование, без
идентичности сервисов и L7-управления между ними, — и mesh не принял.
При таких требованиях реализации, умеющие только north-south, снова
допустимы, и выбор решается доводами настоящего ADR, а не GAMMA.
...
ReadWriteMany-тома поверх NFS или SMB.~~ Условие снято ADR 0002: тома
только ReadWriteOnce.

Оставшийся довод против варианта A — цена самого перехода: разрыв
установленных соединений, зависимость от прямого адреса apiserver и
отсутствие быстрого отката. Вариант B остаётся допустимым, поскольку
east-west от Gateway API не требуется.
...
* ~~Нужны ли платформе ReadWriteMany-тома.~~ **Закрыто в ADR 0002:**
  тома только ReadWriteOnce, NFS и SMB не используются. Ограничение
  socket-LB не применяется, довод против замены kube-proxy отпадает.
* ~~Поддерживает ли выбранная реализация GAMMA.~~ **Закрыто:** Cilium
  conformant по обоим профилям — Gateway и Mesh, начиная с версии 1.19.
  Из реализаций, умеющих east-west, conformant также Istio; Envoy Gateway,
  NGINX Gateway Fabric, Traefik, Kong и прочие покрывают только
  north-south. При требованиях из ADR 0002 это перестало быть решающим.
